...
WHATSAPP_ACCESS_TOKEN="$(aws ssm get-parameter --name "$${PARAM_BASE}/whatsapp/access_token" --with-decryption --region "$${AWS_REGION}" --query 'Parameter.Value' --output text)"
WHATSAPP_PHONE_NUMBER_ID="$(aws ssm get-parameter --name "$${PARAM_BASE}/whatsapp/phone_number_id" --region "$${AWS_REGION}" --query 'Parameter.Value' --output text)"
WHATSAPP_VERIFY_TOKEN="$(aws ssm get-parameter --name "$${PARAM_BASE}/whatsapp/verify_token" --with-decryption --region "$${AWS_REGION}" --query 'Parameter.Value' --output text)"
WHATSAPP_API_VERSION="$(aws ssm get-parameter --name "$${PARAM_BASE}/whatsapp/api_version" --region "$${AWS_REGION}" --query 'Parameter.Value' --output text 2>/dev/null || echo "v21.0")"
META_APP_SECRET="$(aws ssm get-parameter --name "$${PARAM_BASE}/meta/app_secret" --with-decryption --region "$${AWS_REGION}" --query 'Parameter.Value' --output text 2>/dev/null || echo "")"
