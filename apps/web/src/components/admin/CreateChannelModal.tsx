...
export function CreateChannelModal({ onClose }: CreateChannelModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    createMutation.mutate({ name, title, bio });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-2xl">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-gray-900">Criar Novo Canal</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder="Breve descrição sobre o oficial..."
          ...
          {createMutation.isError && (
            <p className="text-sm text-red-600">
              {createMutation.error?.message || "Erro ao criar canal"}
            </p>
          )}
        </form>
      </div>
